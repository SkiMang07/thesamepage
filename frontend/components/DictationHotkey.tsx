...
export default function DictationHotkey() {
  ...
  const { state, seconds, error, clearError, start, stop, cancel } = useDictation({ onText });
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      // Escape while recording discards, and must not also reach the app
      // shell (which closes the Scribe drawer on Escape).
  ...
  }, [supported, state, recording, start, stop, cancel, clearError]);
