...
function SettingsFlow() {
  ...
  const [section, setSection] = useState<SectionId>(
    sectionParam === "people" || sectionParam === "capacity" || sectionParam === "account"
      ? sectionParam
  ...
  // Roles & expectations moved to its own Workspace page. Old links
  // (?section=roles) land there instead of on an empty editor.
  useEffect(() => {
    if (sectionParam === "roles") router.replace("/app/expectations");
  }, [sectionParam, router]);

  function selectSection(next: SectionId) {
    if (next === "roles") {
      router.push("/app/expectations");
      return;
    }
    setSection(next);
    setError(null);
          ...
          <div className="p-5 sm:p-6">
            ...
            {section === "people" && (
              <PeopleSection
                ...
                onNavigateToRoles={() => router.push("/app/expectations")}
                onNavigateToExpectations={() => router.push("/app/expectations")}
                onDraftExpectations={(roleLevelId) => router.push(`/app/expectations/${roleLevelId}`)}
          ...
          >
            ...
            <span className="min-w-0">
              ...
              <span className="mt-0.5 block truncate text-xs text-ink-muted">
                {item.id === "roles"
                  ? `Its own page now · ${foundationBlurb(item.id, profile, setupStatus, roleFamilies, roleLevels, capacity, workUnits)}`
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/55 px-4 pt-24" onClick={onClose}>
      <form onSubmit={submit} className="w-full max-w-sm rounded-xl bg-surface p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-4 flex items-center justify-between gap-2">
          <Link href={pasteJdHref} className="text-xs font-medium text-brand hover:text-brand-hover">
            Define it from a job description
          </Link>
    ...
    <div>
      ...
      {creatingRoleFor && (
        <CreateRoleModal
          ...
          pasteJdHref={
            creatingRoleFor && creatingRoleFor !== "header"
              ? `/app/expectations/new?assign=${creatingRoleFor.id}`
              : "/app/expectations/new"
          }
        />
      )}
      {creatingTeamFor && (
