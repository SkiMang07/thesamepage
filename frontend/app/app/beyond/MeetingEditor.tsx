...
const EMPTY_DRAFT: BeyondWrapUpDraft = { summary: "", commitments: [], check_ins: [], report_notes: [] };
...
export default function MeetingEditor({
  ...
  onLogged: (meeting: OutsideMeetingDetail) => void;
  onDeleted?: () => void;
}) {
  ...
  const [date, setDate] = useState(existing ? isoToDateStr(existing.meeting_date) : localDateStr());
  ...
  async function persist(): Promise<OutsideMeeting> {
    const body = { title: title.trim(), kind, scheduledAt: date || localDateStr(), personIds, notes };
    if (meeting) {
      const updated = await updateOutsideMeeting(meeting.id, body);
      setMeeting(updated);
      return updated;
    }
    const created = await createOutsideMeeting({ ...body, title: body.title || null });
    setMeeting(created);
    return created;
  }
  ...
  if (phase === "review" && meeting) {
    return (
      <PageShell maxWidth="3xl">
        ...
        <div className="mt-5">
          <BeyondWrapUpReview
            meeting={{ ...meeting, people: sortedPeople.filter((p) => personIds.includes(p.id)) }}
            reports={reports}
            goals={goals}
            projects={projects}
            rawNotes={notes}
  ...
  return (
    <PageShell maxWidth="6xl">
      ...
      <h1 className="mt-2 text-2xl font-semibold">{existing ? "Finish writing this up" : "Log a meeting"}</h1>
