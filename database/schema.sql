...
create table metric_configs (
  ...
  expectation          text,
  created_at           timestamptz not null default now(),
  -- Roles & expectations (2026-09-26, docs/systems/expectations.md). All
  -- nullable: rows configured before the workflow keep their meaning.
  -- target_status 'unresolved' = deliberately no target yet (never zero,
  -- never a guessed standard, excluded from numerical evaluation); null =
  -- legacy row whose target, if any, lives in its wording. retired_at is set
  -- when an approved revision drops the item; readers ignore retired rows.
  exceeds              text,
  target               text,
...
create table skill_configs (
  ...
  expectation          text,
  created_at           timestamptz not null default now(),
  -- 2026-09-26: area 'responsibility' = a responsibility judged by
  -- observation rather than a number; null/'skill' = a skill.
  exceeds              text,
  area                 text check (area is null or area in ('responsibility', 'skill')),
  retired_at           timestamptz
);
...
-- ============================================================
-- ROLE EXPECTATION DRAFTS AND OPEN DECISIONS (2026-09-26)
-- ============================================================
-- The working layer in front of the approved *_configs rows: a working draft
-- (new role) or working revision (approved role being refined), and the
-- details a manager chose to come back to. Only
-- approve_role_expectation_draft() publishes a draft. See
-- docs/systems/expectations.md.

create table role_expectation_drafts (
  id            uuid primary key default uuid_generate_v4(),
  org_id        uuid not null references organizations(id) on delete cascade,
...
create unique index role_expectation_drafts_one_open
  on role_expectation_drafts (role_level_id) where status = 'open';
create index role_expectation_drafts_org_idx
  on role_expectation_drafts (org_id, status, updated_at desc);

create table role_expectation_decisions (
  id                uuid primary key default uuid_generate_v4(),
  org_id            uuid not null references organizations(id) on delete cascade,
  role_level_id     uuid not null references role_levels(id) on delete cascade,
...
create index role_expectation_decisions_open_idx
  on role_expectation_decisions (org_id, status, follow_up_on);
create index role_expectation_decisions_role_idx
...
create table goals (
  ...
  due_date         date,
  -- One optional numeric measure per goal (2026-09-25 goals redesign). All
  -- five columns are set together or not at all. The written success_metrics
  -- stays the criterion; this is what a reading is compared against. Readings
  -- live on check_ins.measured_value. measure_format decides validation:
  -- count = whole and >= 0, number = any finite value, percent = any finite
  -- value (NDR runs past 100, so no cap). measure_unit is a display label
  -- only ("handoffs"); it is never parsed. Format and unit are locked once a
  -- reading exists (guard_goal_measure_change below); label, target and
  -- comparison can still be edited, and the UI calls the target "current".
  measure_label     text,
  measure_format    text check (measure_format in ('count', 'number', 'percent')),
  measure_unit      text,
  measure_target    numeric,
  measure_direction text check (measure_direction in ('at_least', 'at_most', 'below')),
  created_at       timestamptz not null default now(),
  constraint goals_measure_complete check (
...
create table check_ins (
  ...
  source_id   uuid,
  -- A reading of the goal's numeric measure (goals.measure_*), entered
  -- explicitly. Null = no new reading; 0 is a real reading. Never derived from
  -- the note, the status or progress. Goal check-ins only.
  measured_value numeric,
  -- Idempotency key from the client. A retried submit with the same key
  ...
  constraint check_ins_measured_value_goal_only
    check (measured_value is null or goal_id is not null),
  constraint check_ins_measured_value_finite
...
-- role_expectation_drafts / role_expectation_decisions (org-scoped like role_levels)
...
-- ============================================================
-- GOAL CHECK-INS — one transaction, retry-safe (2026-09-25)
-- ============================================================
...
-- ============================================================
-- APPROVE A ROLE EXPECTATION DRAFT (2026-09-26)
-- ============================================================
-- The only publication path for role expectations drafted in Roles &
-- expectations: writes every item, retires dropped items, binds/resolves open
-- decisions and marks the draft approved in one transaction. A missing target
-- without a deferred decision, or a stale version, writes nothing.
